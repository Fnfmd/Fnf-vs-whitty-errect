...
local function getCamProperty(cam, p)
	...
	if (str == "default") then
		return p == "scroll.x" and getProperty("camFollow.x") or p == "scroll.y" and getProperty("camFollow.y") or
...
local function setCamProperty(cam, p, v)
	...
	if (str == "default") then
		return p == "scroll.x" and setProperty("camFollow.x", v) or p == "scroll.y" and setProperty("camFollow.y", v) or
...
function shiftCamNote(t, dir)
	if (settings.shiftCameraOnNote) then
		...
		tweenNumber(t, "x", x, 0, (stepCrochet / 1000) * 7, nil, easing.outCubic)
		tweenNumber(t, "y", y, 0, (stepCrochet / 1000) * 7, nil, easing.outCubic)
	end
end
...
function canZooming()
	return getProperty("camZooming") and getCamProperty("zoom") < 1.35 and getPropertyFromClass("ClientPrefs", "camZooms")
end

function onSectionHit()
	if (canZooming()) then
		local ok = .015 * (isDead and 1 or getProperty("camZoomingMult"))
		camZoom = camZoom + ok
		setCamProperty("zoom", getCamProperty("zoom") - ok)
	end
end
...
function onStepHit()
	...
	if (not is060()) then
		if (math.fmod(curStep, 16) == 0 and canZooming()) then
			camZoom = camZoom + .015
			setCamProperty("zoom", getCamProperty("zoom") - .015)
		end
	end
end
...
function onUpdate(dt)
	...
	local pix2 = settings.shiftCameraPixels * .45
...
function onUpdatePost(dt)
	...
	local camZoomingDecay = (isDead or not is060()) and 1 or getProperty("camZoomingDecay")
	...
	if (settings.smoothCamera) then
		...
		camZoom = settings.smoothZoom and math.lerp(camZoom, realCamZoom, math.clamp(dt * 3.125 * camZoomingDecay * smooth, 0, 1)) or realCamZoom
